...
|------|-----|------------|--------|-------------------|---------------|-------------|--------------------|--------------------|------------|-----------------|------------------|---------|-----------|--------------|-------|
| 2026-04-27 | exp-003 | ak-mbp-m1 | pending | Match llama.cpp KV cache precision more closely: round K/V through fp16 on cache write in `src/layers/attention.rs`; keep f16 flash accumulator approximation opt-in only. | pending | pending | pending | pending | pending | pending | pending | pending | pending | preserve llama.cpp-style f16 KV behavior before re-measuring speed | Correctness-focused attention precision change; fill speed fields after next `bench_compare ... --compare-llama` run. |
| 2026-04-27 | exp-002 | ak-mbp-m1 | pending | Harden Q8_0 matmul path in `src/ops/matmul.rs` / `src/ops/quant/quant_K_handler.rs`: remove f32-dequant fallback and Q8 A/B env variants so Q8 linear layers always use activation-quantized Q8×Q8 dot. | pending | pending | pending | pending | pending | pending | pending | pending | pending | preserve llama.cpp-style Q8 execution path before re-measuring speed | Correctness-focused matmul change; fill speed fields after next `bench_compare ... --compare-llama` run. |
